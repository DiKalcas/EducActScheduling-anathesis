<p><strong>Προγραμματισμένα Μαθήματα</strong></p>
<?php
  ...
  try {
    ...
    $sql = "SELECT * FROM eventplannedcourses WHERE schoolID_ofSchools = :schoolID_ofSchools ORDER BY eventPlannedCourseAbbrev ASC";
    //query preparing for avoiding SQL Injection
    $statement= $pdoObject->prepare($sql);
    //passing of the value in placeholders
    $statement->execute( array(':schoolID_ofSchools'=>$_POST['schoolID']));
    
    //query results loop for display
    while ( $record= $statement->fetch() ) {
      //για κάθε record αποτελέσματος κάνε το ακόλουθο
      
      $evePlanCoursID= $record['eventPlannedCourseID'];
      $sql2 = " SELECT COUNT(eventPlannedCourseID) AS rowCounter  
                   FROM eventplannedcourse_scarceresource_participantgroup_combis 
                    WHERE eventPlannedCourseID = $evePlanCoursID "; 
       $statement2= $pdoObject->query($sql2);
       $record2=  $statement2->fetch();
      if ( $record2['rowCounter'] > 0 ) 
      { echo '<p> - '.'<a href="#" onclick="myAJAXCallPlus('.$record['eventPlannedCourseID'].');">' .$record['eventPlannedCourseAbbrev'].'</a>'.'<img src="../checked.png"/><sup>x'.$record2['rowCounter'].'</sup></p>'; } 
      else  
      { echo '<p> - '.$record['eventPlannedCourseAbbrev'].'</p>'; };
    ...
    echo '<p>* Tα τσεκαρισμένα είναι ανατεθημμένα μαθήματα τουλάχιστο μια φορα.</p>';
