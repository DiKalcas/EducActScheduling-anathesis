<?php
  ...
  $record= $statement2->fetch();
      ...
      $myResult1= $statement1->execute( array( ':eventPlannedCourseAbbrev'=>$record[ 'concated' ],
                                             ...
                                             ':ECTS'=>$_POST['ECTS'] ) ); 
       $lastid1= $pdoObject->lastInsertId();                                                                                                             
    }      
    // κλείσιμο PDO
    ...
    $statement2->closeCursor();
      ...
      header('Location: dualform.php?msg=PDO Exception: '.$e->getMessage());
      exit();
  }


  if ( !$myResult1 || !$myResult2  ) {
    header('Location: dualform.php?msg=ERROR: failed to execute sql query');
    exit();
  }
  else {
  
    header("Location: dualform.php?mode=insert&msg=ALL well done!&lastid1=$lastid1");
    exit();
  } 
